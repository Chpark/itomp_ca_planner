...
void PlanningParameters::initFromNodeHandle()
{
  ...
  node_handle.param("max_iterations_after_collision_free", max_iterations_after_collision_free_, 2);
  node_handle.param("num_trajectories", num_trajectories_, 1);
  node_handle.param("trajectory_duration", trajectory_duration_, 5.0);
  node_handle.param("trajectory_discretization", trajectory_discretization_, 0.05);

  node_handle.param("smoothness_cost_weight", smoothness_cost_weight_, 0.0001);
  ...
  node_handle.param("state_validity_cost_weight", state_validity_cost_weight_, 1.0);
  node_handle.param("endeffector_velocity_cost_weight", endeffector_velocity_cost_weight_, 0.0);
  node_handle.param("contact_invariant_cost_weight", contact_invariant_cost_weight_, 1.0);
      ...
      if (segment.size() > 0)
      {
        for (XmlRpc::XmlRpcValue::iterator it = segment.begin(); it != segment.end(); it++)
        {
          ...
          if (it->second.getType() == XmlRpc::XmlRpcValue::TypeString)
          {
            ...
            animate_endeffector_segment_.insert(std::make_pair<std::string, std::string>(component, endeffector));
          }
  ...
  node_handle.param<std::string>("lower_body_root", lower_body_root_, "pelvis_link");
  ...
  if (node_handle.hasParam("joint_velocity_limits"))
  {
    ...
    if (velocity_limits.getType() == XmlRpc::XmlRpcValue::TypeStruct)
    {
      ...
      {
        for (XmlRpc::XmlRpcValue::iterator it = velocity_limits.begin(); it != velocity_limits.end(); it++)
        {
